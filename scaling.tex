...
by instead parameterizing the transition matrix with both generalized softmax
as well as a banded matrix.
In particular, we decompose the logits into a banded matrix and its complement.
We denote the entries $(i,j)$ of the matrix within the band by
\begin{equation}
\mcS = \set{ (i,j) \mid |i - j| < \delta},
\end{equation}
where $\delta$ gives the width of the band,
and we denote the complement of this set, the off-diagonal elements, by $\mcS^c$.
We can then decompose the computation performed in the key step in inference
into an operation over the band plus generalized softmax, and normalize over both.
It is cumbersome to write this operation in matrix notation, so we write it as follows:
\begin{equation}
\begin{aligned}
p(z_t \mid z_{t-1})
&= \sum_{z_{t-1}}p(z_t \mid z_{t-1})p(z_{t-1} \mid x_{<t})\\
&= \left(\sum_{z_{t-1} \in \mcS_{z_t}}p(z_t \mid z_{t-1})
    + \sum_{z_{t-1}\in\mcS^c_{z_t}}p(z_t \mid z_{t-1})\right)p(z_{t-1} \mid x_{<t})\\
&= \sum_{z_{t-1} \in \mcS_{z_t}}p(z_t \mid z_{t-1})p(z_{t-1} \mid x_{<t})
    + \sum_{z_{t-1}\in\mcS^c_{z_t}}p(z_t \mid z_{t-1})p(z_{t-1} \mid x_{<t})\\
&= \sum_{z_{t-1} \in \mcS_{z_t}}\frac{[\theta]_{z_{t-1},z_{t}}}{Z_{z_{t-1}}}p(z_{t-1} \mid x_{<t})
    + \sum_{z_{t-1}\in\mcS^c_{z_t}}
        \frac{\phi(\bu_{z_{t-1}})^\top \phi(\bv_{z_{t}})}{Z_{z_{t-1}}} p(z_{t-1}\mid x_{<t}).
\end{aligned}
\end{equation}
...
\begin{aligned}
Z_{z_{t-1}}
&= \sum_{z_{t-1} \in \mcS_{z_t}} [\theta]_{z_{t-1},z_{t}} + \sum_{z_{t-1}\in\mcS^C_{z_t}}
    \phi(\bu_{z_{t-1}})^\top \phi(\bv_{z_{t}})\\
&= \underbrace{\sum_{z_{t-1} \in \mcS_{z_t}} [\theta]_{z_{t-1},z_{t}}}_{Z_B} + 
    \underbrace{\phi(\bu_{z_{t-1}})^\top \sum_{z_t'\in\mcS^c_{z_t}} \phi(\bv_{z_{t}'})}_{Z_K},
\end{aligned}
\end{equation}
where we denote the contribution to the normalizing constant from the banded matrix as $Z_B$
and the contribution from kernel generalized softmax as $Z_K$.
The term $Z_B$ takes $O(|\mcZ|\delta)$ computation, and $Z_K$ takes $O(|\mcZ|\dim(\mathbb{F}))$.
